...
<%
String isNeedShowPermitKey = Common.get(request.getParameter("isNeedShowPermitKey"));
%>
...
function addGFoodRow(tblId,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10,v11,v12,v13){
	...
	var isNeed = "<%=isNeedShowPermitKey%>";
	...
	if(isNeed == "Y"){
		strGFontLicense.innerHTML += '<input type=button class="field" name=showPermit' + id + ' value="檢視食品資料" onclick="showPermitKey(form1.gPermitKey' + id + '.value,form1.gPermitNo' + id + '.value);">';
	}
	...
	strGIngredient.innerHTML = '<input type=text class=field name=gIngredient' + id + ' size=25 maxlength=40 style="font-size: 10pt" value="' + (v6!=null?v6:'') + '">';
	...
	strGEdibleDate.innerHTML = '<font size="2">起</font><input type=text class=field name=gEdibleDateS' + id + ' onblur="setDateDiff(\'gEdibleDate\',\''+id+'\');" size=7 maxlength=7 style="font-size: 10pt" value="' + (v11!=null?v11:'') + '"><button class=field style="font-size: 10px" type="button" name="btn_gEdibleDateS' + id + '" onClick="popCalendar(\'gEdibleDateS' + id + '&js=setDateDiff(\\\'gEdibleDate\\\',\\\''+id+'\\\')\')" title="萬年曆輔助視窗"><image src="../../images/dynCalendar.gif"></button>' + 
							   '<br>' + 
							   '<font size="2">迄</font><input type=text class=field name=gEdibleDateE' + id + ' onblur="setDateDiff(\'gEdibleDate\',\''+id+'\');" size=7 maxlength=7 style="font-size: 10pt" value="' + (v12!=null?v12:'') + '"><button class=field style="font-size: 10px" type="button" name="btn_gEdibleDateE' + id + '" onClick="popCalendar(\'gEdibleDateE' + id + '&js=setDateDiff(\\\'gEdibleDate\\\',\\\''+id+'\\\')\')" title="萬年曆輔助視窗"><image src="../../images/dynCalendar.gif"></button>' +
							   "<br/><div id='gEdibleDate"+id+"'><div/>";
	...
	setDateDiff('gEdibleDate',id);
}

function showPermitKey(pKey, pNo){
	if(pKey!="" && pNo!=""){
		openCenterWindow(900, 600, "hfrin0412f.jsp?permitKey=" + pKey + "&permitNo=" + pNo);
	}else{
		alert("許可證字號不完整，請重新輸入");
	}
}

function getPermitKey(fieldK, fieldN, fieldC, fieldE, fieldI){
	openCenterWindow(900, 600, "../hfrex/hfrex0112f.jsp?field1=" + fieldK + "&field2=" + fieldN + "&field3=" + fieldC + "&field4=" + fieldE + "&field5=" + fieldI);
}
