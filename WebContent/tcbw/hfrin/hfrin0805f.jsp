...
<jsp:useBean id="obj" scope="request" class="com.kangdainfo.tcbw.view.hfrin.HFRIN0804F">
...
obj = (com.kangdainfo.tcbw.view.hfrin.HFRIN0804F)obj.queryOne();
%>
<html>
<head>
...
function checkField(){
	var alertStr = "";
	alertStr += validateTabFileHFRRE();
	
	if(form1.state.value == "doComplete"){
		<%=TCBWCommon.getCheckFiled("HFR01", "HFR08")%>
		alertStr += checkHFR0008FDate();
	}
	if(alertStr.length!=0){ alert(alertStr); return; }
	
	if(form1.state.value == "doComplete" && form1.reEvaluateResult.value == "1"){
		if(form1.isReactionResult[0].checked){
			openCenterWindow(800, 300, 'hfrin0806f.jsp');
		}else{
			goSubmit();
		}
	}else{
...
function checkHFR0008FDate(){
	var alertStr = "";
	$("#Tab5 :text[name$=Date][class=field]").each(function(){		
...
function init(){
	if("<%=obj.getIsNeedBackQuery()%>" == "N" || "<%=obj.getIsNeedBackQuery()%>" == ""){
		...
		setFormItem("pauseSave,doComplete,doCommittee","R");
...
<div id="queryContainer" style="width:700px;height:450px;display:none">
	...
	<jsp:include page="hfrin0801q.jsp" />
</div>
...
<tr>
	...
	<td ID="t2" CLASS="tab_border2"><a id="aTab2" href="#" onClick="changeTab(2);">調查紀錄</a></td>		
	...
	<td ID="t6" CLASS="tab_border2"><a id="aTab6" href="#" onClick="changeTab(6);">初步評估</a></td>
	<%
	if(!"".equals(obj.getHfr0006Id())){
	%>
	<td ID="t7" CLASS="tab_border2"><a id="aTab7" href="#" onClick="changeTab(7);">預評作業</a></td>
	<%
	}
	%>
	<td ID="t5" CLASS="tab_border2"><a id="aTab5" href="#" onClick="changeTab(5);">複評作業</a></td>
...
localButtonFireListener.whatButtonFireEvent = function(buttonName){
	...
	case "pageUpdate":
		...
		if(flag){
			...
			chThiSeverity(form1.thiSeverity.value);
