...
<jsp:useBean id="obj" scope="request" class="com.kangdainfo.tcbw.view.trans.TRANSVDRG">
...
else if (obj.getState().equals("importTemplate02") && !"".equals(obj.getQ_filePath())) 
{
	    obj.setPeople(User.getUserId());
		obj.trans02();	
		obj.setErrorMsg(obj.getMsg());
}
else if (obj.getState().equals("deleteTemplate")) 
{	    
		obj.deleteTrans();	
		obj.setErrorMsg(obj.getMsg());
}
...
{
	var alertStr="";
	alertStr += checkEmpty(form1.transType,"匯入檔案類別");
	if(alertStr.length!=0){ 
		form1.state.value = "";
		alert(alertStr);
		return false;
	}	
	if("01"==form1.transType.value){
		form1.state.value = "importTemplate";
	}else{
		form1.state.value = "importTemplate02";
	}	
	form1.submit();
	//show();
}
...
function doDeleteProcess() {
	var alertStr="";
	//alertStr += checkEmpty(form1.deleteYM,"獲知日期年月");
	if(alertStr.length!=0){ 
		form1.state.value = "";
		alert(alertStr);
		return false;
	}
	form1.state.value = "deleteTemplate";
	form1.submit();		
}
</script>
</head>
...
<tr>	
  <td class="td_form">匯入檔案類別：</td>
  <td class="td_form_white" colspan="3">
  <select name="transType" class="field" >
     <%=View.getTextOption("01;103年03月前;02;103年03月後(含);", obj.getTransType(), 1) %>
  </select>
  </td>		
</tr>
<tr>	
...
<tr>
<td class="td_form">獲知日期年月：</td>
  <td class="td_form_white" colspan="3">
     <input type="text"   class="field"  name="deleteYM"  size="5" maxlength="5" value="" /> (ex:10301)
     <input type="button" class="toolbar_default" name="btndelete" id="btndelete" value="刪除匯入資料" onclick="doDeleteProcess();" />
  </td>
</tr>
</table>
<br>
