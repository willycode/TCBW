...
<jsp:useBean id="obj" scope="request" class="com.kangdainfo.tcbw.view.drgex.DRGEX0104F">
...
if ("true".equals(obj.getQueryAllFlag())){
    ...
    if(permitKey!=""){ 
    	obj.setQ_permitKey(permitKey);
    }else{
    	obj.setQ_permitKey("01"); //先預設一筆
    }
...
function checkField(type){
	if(type == "confirm"){
		if (opener!=null) {
			if (form1.id.value == "") alert("請先點選一筆資料!\n");
			else{
				if(null != form1.q_keyId.value && "" != form1.q_keyId.value){
					opener.setPermitData(form1.id.value, form1.q_keyId.value);
				}else{
					opener.permitData2(form1.id.value, form1.q_keyId.value);
				}
				window.close();
			}
		}
...
<table width="100%" cellspacing="0" cellpadding="0" style="width:auto;height:auto;">
	...
	<tr>
		<td nowrap class="queryTDLable">藥品中文品名：</td>
		<td nowrap class="queryTDInput">
			<input class="field_Q" type="text" name="q_chProduct" size="100" maxlength="100" value="<%=obj.getQ_chProduct()%>">
	    </td>
	</tr>
	<tr>
		<td nowrap class="queryTDLable">藥品英文品名：</td>
		<td nowrap class="queryTDInput">
			<input class="field_Q" type="text" name="q_enProduct" size="100" maxlength="200" value="<%=obj.getQ_enProduct()%>">
	    </td>
	</tr>	
</table>	
...
<input type="hidden" name="q_keyId" value="<%=obj.getQ_keyId()%>">
