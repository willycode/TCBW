...
function addFile<%=fileType%>(tblId,v1,v2,v3,v4)
{
    ...
    strFileName.innerHTML = '<input type=hidden name=' + fType + 'FileRow value=' + id + '>' + 
    						...
    						'<input type=button class="field_Q" name=btn_' + fType + 'FileName' + id + 'Download onclick="openDownloadWindow' + fType + '(form1.' + fType + 'FileName' + id + '.value, form1.'+ fType + 'FileNameRoute' + id + '.value);" value=\'下載檔案\'>';
...
function openUploadWindow<%=fileType%>(popFieldFileName, popFieldFileRoute, fileShowName)
{
	if (isObj(eval("form1." + popFieldFileName))) 
	{
	    ...
	    win = window.open('../../home/uploadFileHiddenRoute.jsp?popFieldFileName=' + popFieldFileName + '&popFieldFileRoute=' + popFieldFileRoute + "&popFileType=PROCOND&fileShowName=" + fileShowName,'上傳檔案',prop);
	} 
	else 
	{
		alert("欄位不存在，請檢查!");
...
function openDownloadWindow<%=fileType%>(fileName, fileRoute, fileShowName)
{
    var prop='';
	...
	if (fileName!=null && fileName.length>0) {
	    var url = getVirtualPath() + "downloadFileSimple?fileID=" + fileRoute + ':;:' + fileName + "&fileType=PROCOND";
	    if(win != null) win.close();
...
function removeRowInTable<%=fileType%>(tblId, rid, fileName, fileRoute, id)
{
	...
	// 判斷是否刪除位儲存的檔案
	if(id.indexOf("PROCOND") != -1 || id.indexOf("P") != -1)
	{
		$.ajax({
			url: '../../ajax/jsonDeleteTmpFile.jsp',
			type: 'POST',
			data: 'fileID=' + fileRoute + ':;:' + fileName +"&fileType=PROCOND",
			dataType: 'json',
