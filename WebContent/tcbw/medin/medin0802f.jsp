...
<jsp:useBean id="objQry0801" scope="request" class="com.kangdainfo.tcbw.view.medin.MEDIN0801F">
	<jsp:setProperty name="objQry0801" property="*"/>
</jsp:useBean>
...
if("1".equals(obj0802.getCaseType()))
...
if("pauseSave".equals(obj0802.getState()) || "upload".equals(obj0802.getState()))
{
	try
	{
		obj0802.doUpdateType0802();
		obj0802.setErrorMsg("暫存成功");
	}
	catch(Exception e)
	{
		e.printStackTrace();
		obj0802.setErrorMsg("發生未預期錯誤，請重新操作 !");
	}
}
else if("completed".equals(obj0802.getState()))
{
	try
	{
		obj0802.doCompleted();
		obj0802.doDeleteCon0003Db();
		obj0802.setErrorMsg("評估完成");
	}
	catch(Exception e)
	{
		e.printStackTrace();
...
else if("completedConfirmed".equals(obj0802.getState()))
{
	try
	{
		obj0802.doCompletedConfirmed();
		obj0802.doDeleteCon0003Db();
		obj0802.setErrorMsg("評估確認完成");
	}
	catch(Exception e)
	{
		e.printStackTrace();
...
else if("toClear".equals(obj0802.getState()))
{
	try
	{
		obj0802.setHttpRequest(pageContext.getRequest());	
		obj0802.doDeleteCon0003Db();
	}
	catch(Exception e)
	{
...
function checkField()
{
	...
	alertStr +=validateAddrTable('med1007DbRow','patientIssuesNameRe');
	...
	if(form1.state.value == "completed")
	{
		...
		<%=TCBWCommon.getCheckFiled("MED02","MEDIN0802F")%>
	}
...
function init() 
{
	...
	//只有不良反應評估頁籤可以修改
	$('#Tab7').find('* :input').attr('class','field_U');
	
	setDisplayItem('spanInsert,spanQueryAll,spanDelete,spanConfirm,spanListPrint,spanListHidden','H');

	setFormItem("pauseSave,completed,doUpload,doUpload0703,doUpload0803,doUploadForReplayContent","R");
    ...
    if('<%=obj0802.getErrorMsg()%>'=='評估完成' || '<%=obj0802.getErrorMsg()%>'=='評估確認完成' )
	{	
	    form1.action = "medin0801f.jsp?statusType="+'<%=statusType%>'+"&caseType="+'<%=obj0802.getCaseType()%>';
...
{
	form1.action = "medin0801f.jsp?statusType="+'<%=statusType%>'+"&caseType="+'<%=caseType%>';
	form1.state.value = "queryAll";
	form1.submit();
}
...
<div id="queryContainer" style="width:700px;height:450px;display:none">
	...
	<% request.setAttribute("qBean", objQry0801); %>
...
<tr>
	...
	<input type="hidden" name="isCloseUserInfo" value="<%=obj0802.getIsCloseUserInfo()%>"> <!-- 列印時是否遮蔽個資 -->
    ...
    <%if("75".equals(obj0802.getStatusType())){%>
    ...
    <%if("76".equals(obj0802.getStatusType())){%>
...
localButtonFireListener.whatButtonFireEvent = function(buttonName)
{
	switch (buttonName)	
	{
	  case "doQuery":
		  ...
		  break;
	  case "update":
		  setFormItem("pauseSave,completed,doUpload,doUpload0703,doUpload0803,doUploadForReplayContent","O");
	  ...
	  case "doMailList":
		   <%if("1".equals(obj0802.getCaseType())) {%>
			popCon0002('MED01','<%=obj0802.getId()%>');
			<%} else if("2".equals(obj0802.getCaseType())) { %>
			popCon0002('MED02','<%=obj0802.getId()%>');
			<%} else {%>
			popCon0002('MED','<%=obj0802.getId()%>');
			<%}%>
			break;
	  case "doFlow":
		  <%if("1".equals(obj0802.getCaseType())) {%>
