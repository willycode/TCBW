...
<jsp:useBean id="obj" scope="request" class="com.kangdainfo.tcbw.view.pmed.PMED0301Q">
...
{
	obj = (com.kangdainfo.tcbw.view.pmed.PMED0301Q)obj.queryOne();
}
obj.setQueryAllFlag("true"); 
...
function queryOne(id,id2)
{
    ...
    if("03" == form1.caseType.value)
    {
    	form1.action = "pmed0201f.jsp";	//待補件案件在這支程式進行補件
    }
    else
    {
...
<tr><td nowrap class="bg">
	...
	<tr>
		...
		<td nowrap class="td_form_white"  colspan="3">
			<%=View.getPopCalendar("field_Q","notifierRevDate",obj.getQ_monitorSDate())%>~
			<%=View.getPopCalendar("field_Q","notifierRevDate",obj.getQ_monitorSDate())%>
		</td>	
	</tr> 
	<tr>
	...
	<tr>
		...
		<td nowrap class="td_form_white" >
			...
			~
			<%=View.getPopCalendar("field_Q","q_prehanddateE",obj.getQ_prehanddateS())%>
		</td>		
		<td nowrap class="td_form" >實際繳交日期</td>
		<td nowrap class="td_form_white">
			<%=View.getPopCalendar("field_Q","q_handdateS",obj.getQ_handdateS())%>
			~
			<%=View.getPopCalendar("field_Q","q_handdateE",obj.getQ_handdateS())%>
		</td>
	</tr>
	<tr>
