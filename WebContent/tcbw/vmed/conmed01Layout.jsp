...
function addConRow<%=filetype%>(tblId,v1,v2,v3,v4){
	...
	conItem.innerHTML = '<input type=hidden name=conRow<%=filetype%> value=' + id + '>'+
...
function openDownload(fileRoute){
	if (null != fileRoute && "" != fileRoute) {
	    ...
	    closeReturnWindow();
	    returnWindow = window.open('../../downloadFileSimple?fileType=MED&fileID='+encodeURI(encodeURI(fileRoute)),'上傳檔案',prop);
	} else {
		alert("欄位不存在,請檢查!");
		return ;
	}
...
function upload(dbName, systemType,id){
