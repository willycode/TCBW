...
function openUploadWindow<%=fileType%>(popFieldFileName, popFieldFileRoute, fileShowName){
	if (isObj(eval("form1." + popFieldFileName))) {
	    ...
	    win = window.open('../../home/uploadFileHiddenRoute.jsp?popFieldFileName=' + popFieldFileName + '&popFieldFileRoute=' + popFieldFileRoute + "&popFileType=COS&fileShowName=" + fileShowName,'上傳檔案',prop);
	} else {
		alert("欄位不存在，請檢查!");
		return ;
	}
...
function removeRowInTable<%=fileType%>(tblId, rid, fileName, fileRoute, id){
	...
	// 判斷是否刪除位儲存的檔案
	if(id.indexOf("COS") != -1 || id.indexOf("C") != -1){
		$.ajax({
