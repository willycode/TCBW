...
var idCOS0005DbCount = 0;
var arrCOS0005DbFieldName = new Array("cName","manufactorName","useDateS","useDateE","useRate","useMethod","manufactorNo","expirationDate","tradeDate");
var arrCOS0005DbFieldZhName = new Array("","","使用期間起","使用期間迄","","","","保存期限","購買日期");

function addCOS0005DbRow(tblId,v1,v2,v3,v4,v5,v6,v7,v8,v9,v10){
	$("#COS0005DbListTFOOT").remove();
	
	idCOS0005DbCount += 1;
	var tbl = document.getElementById(tblId);
	var lastRow = tbl.rows.length;
	var row = tbl.insertRow(lastRow);
	var id = v10!=null?v10:("tmp" + idCOS0005DbCount);
	
    var strCName = row.insertCell(0);
    strCName.innerHTML = '<input type=hidden name=COS0005DbRow value=' + id + '><input type=text class=field name=cName' + id + ' size=20 maxlength=90 style="font-size: 10pt" value="' + (v1!=null?v1:'') + '">';
	...
	sb.append('<input style="font-size: 11px" type="button" class="field_btnRemove" value="" onclick="removeRowFromTable(\'' ).append( tblId ).append( '\',this.parentNode.parentNode.rowIndex);createCOS0005DbListTFOOT();">');
	sb.append('<input style="font-size: 11px" type="button" class="field_btnAdd" value="" onclick="copyCOS0005DbRow(\'' ).append( tblId ).append( '\',\'' + id + '\');">');
	actionText.innerHTML = sb.toString();
}

function copyCOS0005DbRow(tblId, rid) {
	var arrFd = new Array(9);
	for(var i=0; i<arrFd.length; i++){		
		if(isObj(document.all.item(arrCOS0005DbFieldName[i] + rid))){
			arrFd[i] = document.all.item(arrCOS0005DbFieldName[i] + rid).value;
		}
	} 	
	addCOS0005DbRow(tblId, arrFd[0], arrFd[1], arrFd[2], arrFd[3], arrFd[4], arrFd[5], arrFd[6], arrFd[7], arrFd[8]);
}

function validateCOS0005DbTable() {
	var COS0005DbArray = document.getElementsByName("COS0005DbRow");	
	if(COS0005DbArray!=null && COS0005DbArray.length>0){	
	
		for(var i=0; i<COS0005DbArray.length; i++){
			var id = COS0005DbArray[i].value;
			
			var sb = new StringBuffer();
			for(var j=0; j<arrCOS0005DbFieldName.length; j++){
				var fd	= document.all.item(arrCOS0005DbFieldName[j] + id);
				if(isObj(fd) && arrCOS0005DbFieldZhName[j] != "") {
					sb.append(checkDate(fd, arrCOS0005DbFieldZhName[j]));
				}
			}
...
function createCOS0005DbListTFOOT(){
	var tbl = $("#tabCOS0005Db").get(0);
	if(tbl.rows.length == 1){
		var t = $("<tfoot id='COS0005DbListTFOOT'><tr><td>&nbsp;</td></tr></tfoot>");
		$(tbl).append(t);
	}
}
</script>

<table width="100%" class="table_form" cellspacing="0" cellpadding="0" id="tabCOS0005Db">
	<thead id="COS0005DbListTHEAD">
	...
	<th class="listTH">
		<font size="2">
			<input type="button" class="field_btnAdd" onclick="addCOS0005DbRow('tabCOS0005Db');" />
		</font>
	</th>
	</thead>
	
	<tbody id="COS0005DbListTBODY">
	</tbody>
	
	<tfoot id="COS0005DbListTFOOT">
		<tr><td>&nbsp;</td></tr>
	</tfoot>
</table>
