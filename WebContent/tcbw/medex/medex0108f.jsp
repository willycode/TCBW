...
String b = Common.get(request.getParameter("b"));
String c = Common.get(request.getParameter("c"));
String d = Common.get(request.getParameter("d"));
			...
			else
			{
				...
				else
				{
					...
					if(form1.rowCount.value != "" && !isNaN(form1.rowCount.value))
						...
						opener.manyPermitData2(form1.id.value, form1.q_keyId.value , form1.rowCount.value,'<%=b%>','<%=c%>','<%=d%>');
...
<table class="table_form" width="100%" cellspacing="0" cellpadding="0" id="tbl">
	...
	out.write(View.getQuerylist(
			primaryArray,displayArray,
			null,objList,
			"",true,
			null,"",
			"",false,
