...
function queryOne(id,codeKindName,codeID,codeName,codeEngName,codeCon1,codeCon2,codeCon3,memo,editID,editDate){
	...
	if (isObj(opener.document.all.item("<%=obj.getChangeField()%>"))) {
		opener.document.all.item("<%=obj.getChangeField()%>").value=codeEngName.substring(0,2);
...
<%if("CONPUBLDEPT".equals(obj.getQ_codeKindID())){%>
	...
	<tr>
		...
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',3,false);" href="#">單位名稱</a></th>
		<th class="listTH"><a class="text_link_w" onclick="return sortTable('listTBODY',4,false);" href="#">單位國家</a></th>
	</tr>
	</thead>
	<tbody id="listTBODY">
		<%
		boolean primaryArray[] = {true,  true, true, true, true, true, true, true, true};
		boolean displayArray[] = {false, true,  true, true, true, false, false, false, false, false};
