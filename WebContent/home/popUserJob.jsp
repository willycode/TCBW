...
String y = Common.get(request.getParameter("y"));
String p = Common.get(request.getParameter("p"));  //表單別

if(!"".equals(v)){
	obj.setType(v);
}
if(!"".equals(y)){
	obj.setIsNonNeedGetBack(y);
}
if(!"".equals(p)){	
	obj.setFormdName(p);
}
...
function queryOne(id, userJobName, userJobType, cityZip, address, city, zip)
{
	...
	<%
    if(!"Y".equals(obj.getIsNonNeedGetBack())){
    %>
	...
	//重大品質事件廠商主動通報 
	<%if("SDRG0101".equals(obj.getFormdName())){%>	
		if (isObj(opener.document.all.item("appID")))		
			opener.document.all.item("appID").value=userJobType;	
		if (isObj(opener.document.all.item("appName")))		
			opener.document.all.item("appName").value=userJobName;	
		if (isObj(opener.document.all.item("appAddr")))		
			opener.document.all.item("appAddr").value=cityZip+address;
	<%}%>
	<%if("SDRG0201_1".equals(obj.getFormdName())){%>	
		if (isObj(opener.document.all.item("healthbureau1")))		
			opener.document.all.item("healthbureau1").value=id;	
		if (isObj(opener.document.all.item("healthbureau1Name")))		
			opener.document.all.item("healthbureau1Name").value=userJobName;
	<%}%>
	<%if("SDRG0201_2".equals(obj.getFormdName())){	%>
		if (isObj(opener.document.all.item("healthbureau2")))		
			opener.document.all.item("healthbureau2").value=id;	
		if (isObj(opener.document.all.item("healthbureau2Name")))		
			opener.document.all.item("healthbureau2Name").value=userJobName;
	<%}%>
	<%if("SDRGQuery".equals(obj.getFormdName())){%>
	    if (isObj(opener.document.all.item("q_healthbureau")))		
		    opener.document.all.item("q_healthbureau").value=id;	
	    if (isObj(opener.document.all.item("q_healthbureauName")))		
		    opener.document.all.item("q_healthbureauName").value=userJobName;
    <%}%>
...
<input type="hidden" name="isNeedGetBack" value="<%=obj.getIsNonNeedGetBack()%>">
<input type="hidden" name="id" value="<%=obj.getId()%>">
<input type="hidden" name="formdName" value="<%=obj.getFormdName()%>">
