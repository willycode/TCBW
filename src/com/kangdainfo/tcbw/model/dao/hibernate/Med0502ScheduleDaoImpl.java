...
public class Med0502ScheduleDaoImpl extends AbstractScheduleDaoImpl implements 	ScheduleDao {
	...
	public MsgObject executeSchedule() throws Exception {
		...
		logger.info("======== 啟動Med0502醫療器材定期安全監視表報告繳交通知(到期當日)作業排程 ========");
		try {
			...
			if(objList != null && objList.size()>0){
				...
				for(Object dtlObj : objList)
				{
					...
					Con1001Db c = (Con1001Db)View.getObject("from Con1001Db where mailID='MED050002'");									
				...
				logger.info("無醫療器材定期安全監視表報告繳交通知(到期當日)案件");
			}			
		} catch (Exception e) {
			e.printStackTrace();
			String errorMessage = "啟動Med0502排程失敗" + e.getMessage();
			logger.info(errorMessage);
			resultMap.put("2", errorMessage);			
			r.setReturnMap(resultMap);
			r.setReturnMsg(errorMessage);
		}
		logger.info("======== 結束Med0501醫療器材定期安全監視表報告繳交通知(到期當日)作業排程 ========");
		return r;
	}
