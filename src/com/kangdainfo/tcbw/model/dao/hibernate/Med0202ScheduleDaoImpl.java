...
public class Med0202ScheduleDaoImpl extends AbstractScheduleDaoImpl implements 	ScheduleDao {
	...
	public MsgObject executeSchedule() throws Exception {
		...
		logger.info("======== 啟動Med0202醫療器材不良品案件複評稽催作業排程 ========");
		try {			
			String today = Datetime.getYYYMMDD();		
			
			/**
			 *   案件狀態為「案件評估中(複評)」，不良事件類別為「不良品」。(案件編號MR02開頭)
			 *   A 級案件的回覆作業期限為10天，以轉送日期(最新一筆)起算，後續每日持續發送稽催信件。
			 *   B 級案件的回覆作業期限為1個月，以轉送日期(最新一筆)起算，後續每週持續發送稽催信件。
			 *   系統發送「案件初評稽催通知」給案件評估者。
             *   如果案件的不良品事件等級為「A級」，郵件需副件密本給TFDA人員。
			 **/			
			String hql1 =" select a.med0001Db.applNo,a.transDate from Med0006Db a where 1=1"+ 
			             " and a.med0001Db.status ='75' and a.med0001Db.applNo like 'MR02%' and a.transType='3' "+
			...
			if(objLists != null && objLists.size()>0){
				...
				String days = View.getLookupField("select field7 from Sys0001Db where id=4"); //不良品複評稽催期限(A級)
				String mons = View.getLookupField("select field8 from Sys0001Db where id=4"); //不良品複評稽催期限(B級)
				...
				while (it.hasNext()) {
					...
					//不良品事件等級
					String eventClass = View.getLookupField("select eventClass from Med0009Db where med0001Db.applNo="+Common.sqlChar(Common.get(obj[0]))+" order by id desc");
					
					if("1".equals(Common.get(eventClass))){ //A
						int transDays = Common.getInt(Datetime.DateSubtraction(Common.get(obj[1]), today));
					...
					if(isMail){
						...
						if(med01 != null){
							...
							Con1001Db c = (Con1001Db)View.getObject("from Con1001Db where mailID='MED020006'");									
				...
				logger.info("無醫療器材不良品案件複評稽催案件");
			}			
		} catch (Exception e) {
			e.printStackTrace();
			String errorMessage = "啟動Med0202排程失敗" + e.getMessage();
			logger.info(errorMessage);
			resultMap.put("2", errorMessage);			
			r.setReturnMap(resultMap);
			r.setReturnMsg(errorMessage);
		}
		logger.info("======== 結束Med0202醫療器材不良品案件複評稽催作業排程 ========");
		return r;
	}
