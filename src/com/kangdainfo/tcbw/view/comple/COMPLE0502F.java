...
public class COMPLE0502F extends COMPLE0501F
{
	...
	{
		ServiceGetter.getInstance().getTcbwService().getComple05Dao().updateByCOMPLE0502F(this);
	}
...
//		System.out.println("PK: " + rowArray2);
	...
	public Object doQueryOne() throws Exception 
	{
	  ...
	  {	
		  ...
		  obj.setMed0001ID(Common.get(c.getId()));
		  ...
		  //obj.setNotifierFirmDept(c.getnotifier);//通報者屬性-廠商-服務機構
	...
	public String getAddFile() throws Exception 
	{
		...
		String hql = " from Con0001Db where fileKind='MED' and systemType like 'MED%' and dbName='Med0001Db' and upLoadId="+getId();
	...
	{
		Med0001Db obj = (Med0001Db)View.getObject(" from Med0001Db where id = " + Common.getLong(getId()));
		if(obj != null)
		{
	...
	{		
		if("1".equals(type)){ //刪除全部檔案
			String hql = " from Con0001Db where fileKind='MED' and systemType like 'MED010001' and dbName='Med0001Db' and upLoadId="+id;		
			java.util.List objList = ServiceGetter.getInstance().getCommonService().load(hql+" order by id asc");		
			if(objList!=null && objList.size()>0)		
			{			
				java.util.Iterator it = objList.iterator();			
			...
			String hql = " from Con0001Db where fileKind='MED' and systemType like 'MED010001' and dbName='Med0001Db' and isInsert='Y' and upLoadId="+id;			
			java.util.List objList = ServiceGetter.getInstance().getCommonService().load(hql+" order by id asc");			
			if(objList!=null && objList.size()>0)			
			{				
				java.util.Iterator it = objList.iterator();				
