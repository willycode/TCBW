...
public class COMPLE0501F extends SuperBean
{
	...
	{		
		...
		String hql = " from Med0001Db where 1 = 1 and trans = 'Y'";
		...
		hql+=" and ( id in ( select max(id) from Med0001Db where applNo != null  group by applNo )"+
		     "  or id in ( select max(id) from Med0001Db where applNo is null group by id) )";
		
		System.out.println("COMPLE0501F=="+hql);
		...
		if (getTotalRecord() > 0)
		{
			...
			if (objList != null && objList.size() > 0) 
			{
				...
				{				
					...
					rowArray[5] = Common.get(medMainCategoryMap.get(dtl.getMedMainCategory()));
