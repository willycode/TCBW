...
public class VCOS0301F extends SuperBean
{
	private String q_id;
	...
	public void setQ_isImport(String q_isImport) {this.q_isImport = checkSet(q_isImport);}	
	public String[] getQ_lamp() {return checkGet(q_lamp);}
	public void setQ_lamp(String[] q_lamp) {this.q_lamp = checkSet(q_lamp);}
	...
	{		
		...
		String hql = " from Cos7001Db where  status ='10'  ";
		...
		if(!"".equals(Common.get(getQ_lamp()))){
			String glamp = "";
			for(String o: getQ_lamp()){
				if(glamp.length() > 0)	glamp += ",";
				glamp += Common.sqlChar(o);
			}
			hql += " and lamp in (" + glamp + ")";
		}
		
		java.util.List objList = ServiceGetter.getInstance().getTcbwService().load(hql + " order by applNo desc");
