...
public class PMED0301Q extends SuperBean
{
	...
	{		
		...
		String compegno=View.getLookupField(" select con1005Db.compegno from Con1006Db where commonUser.id="+ Common.getLong(getUserID()));
...
//		}
		...
		if(!"".equals(getCaseType())) {
			hql += " and b.handstatus = " + Common.sqlChar(getCaseType());
		}
		...
		System.out.println("[TCBW]-[PMED0301Q]-[QUERYALL] : " + hql);
...
//		java.util.List objList = null;
		...
		if (getTotalRecord() > 0) 
		{
			...
			objList = ServiceGetter.getInstance().getTcbwService().load( hql + " order by b.id desc", this.getRecordStart(), this.getPageSize());
