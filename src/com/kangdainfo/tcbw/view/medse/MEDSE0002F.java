...
public class MEDSE0002F extends SuperBean{
	...
	public Object doQueryOne() throws Exception {
		MEDSE0002F obj = this;
		Med1003Db c = (Med1003Db)View.getObject("from Med1003Db where id = " + Common.getLong(getId()));		
		if(c != null){
	...
	public Object doQueryAll() throws Exception {
		...
		String hql = " from Med1003Db where 1 = 1 ";
		if(!"".equals(getQ_id())){
		...
		if (getTotalRecord() > 0){
			...
			if (objList != null && objList.size() > 0) {
				for(Object dtlObj : objList) {				
					...
					rowArray[1] = Common.get(dtl.getNum());	
					rowArray[2] = Common.get(dtl.getMatter());	
					rowArray[3] = (Common.get(dtl.getIsYesType()).equals("0")?"+":Common.get(dtl.getIsYesType()).equals("1")?"-":"") + Common.get(dtl.getIsYes());	
					rowArray[4] = (Common.get(dtl.getIsNoType()).equals("0")?"+":Common.get(dtl.getIsNoType()).equals("1")?"-":"") + Common.get(dtl.getIsNo());	
					rowArray[5] = (Common.get(dtl.getIsUnknownType()).equals("0")?"+":Common.get(dtl.getIsUnknownType()).equals("1")?"-":"") + Common.get(dtl.getIsUnknown());	
					rowArray[6] = Common.get(dtl.getSeq());		
	...
	public void doUpdate() throws Exception {
		Med1003Db obj = (Med1003Db)View.getObject(" from Med1003Db where id = " + Common.getLong(getId()));
		if(obj != null){	
			obj.setNum(getNum());
			obj.setMatter(getMatter());
			obj.setIsYes(Integer.valueOf(getIsYes()));
