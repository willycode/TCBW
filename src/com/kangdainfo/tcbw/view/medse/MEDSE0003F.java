...
public class MEDSE0003F extends SuperBean{
...
//				java.util.Map<String, String> sysMap = TCBWCommon.getCommonCodeMap("SYS");
				...
				for(Object dtlObj : objList) {	
					...
					if(dtl.getDefinition().length() > 100) {
						rowArray[4] = Common.get(dtl.getDefinition().subSequence(0, 100));
					} else {
	...
	public void doUpdate() throws Exception {
		Med1005Db obj = (Med1005Db)View.getObject(" from Med1005Db where id = " + Common.getLong(getId()));
		...
		if(obj != null){		
			...
			java.util.List listLv2 = ServiceGetter.getInstance().getCommonService().load(" from Med1005Db where level1Code = " + Common.sqlChar(lv1Code));
			if(listLv2 != null && listLv2.size() > 0) {
				for (int i=0; i<listLv2.size(); i++) {
			    	Med1005Db objLv2 = (Med1005Db) listLv2.get(i);
