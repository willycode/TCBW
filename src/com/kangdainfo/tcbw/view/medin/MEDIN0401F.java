...
public class MEDIN0401F extends SuperBean
{
...
//		}
		...
		System.out.println("[TCBW]-[MEDIN0401F]-[QUERYALL] : " + hql);
		...
		if (getTotalRecord() > 0)
		{
			...
			if (objList != null && objList.size() > 0) 
			{
				...
				{				
                    ...
                    if(sys0001Db!=null)
                    {
					  if("01".equals(dtl.getBadReactionResults()))
					  {
						String date = Datetime.getDateAdd("d",Integer.valueOf(sys0001Db.getField2()),dtl.getNotifierRevDate());
						if(Common.getInt(Datetime.getYYYMMDD()) > Common.getInt(date)) light="<img width='30px' src='../../images/ballRed.png'>";
					  }	
					  else
					  {
						String date = Datetime.getDateAdd("m",Integer.valueOf(sys0001Db.getField3()),dtl.getNotifierRevDate());
						if(Common.getInt(Datetime.getYYYMMDD()) > Common.getInt(date)) light="<img width='30px' src='../../images/ballRed.png'>";
					  }	
                    }
					rowArray[1] = light;	
					...
					rowArray[3] = Common.get(dtl.getNotifierRevDate());
					...
					{
						rowArray[7] = Common.get(status2Map.get(dtl.getStatus()));
					}
					else
					{
						rowArray[7] = Common.get(status3Map.get(dtl.getStatus()));
					}

					rowArray[8] = Common.get(dtl.getApplNo());
					rowArray[9] = Common.get(dtl.getWorkers());
