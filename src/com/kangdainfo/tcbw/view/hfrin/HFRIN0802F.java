...
public class HFRIN0802F extends HFRIN0801F implements HFRIN0001F {
				...
				for(Object dtlObj : c.getHfr0007Dbs()){
					...
					if("Y".equals(Common.get(dtl.getIsClosed()))){
						...
						if(flag){
							...
							obj.setThiUnExpectedClassify(dtl.getUnExpectedClassify());
							if(!"".equals(Common.get(dtl.getUnExpectedReason()))){
								String[] t = Common.get(dtl.getUnExpectedReason()).split(",");
							...
							obj.setThiEvidentiary(dtl.getEvidentiary());
							obj.setThiSeverity(dtl.getThiSeverity());
							obj.setThiAdministrativeLevel(dtl.getAdministrativeLevel());
	...
	public void doUpdateType() throws Exception{
		ServiceGetter.getInstance().getTcbwService().getHfrinDao().updateByHFRIN0802F(this);
	}
