...
public class SDRG0401Q extends SuperBean{
	...
	public Object doQueryAll() throws Exception {		
		...
		if("1".equals(Common.get(getCaseType())) || "2".equals(Common.get(getCaseType()))){ //回收確認-廠商,衛生局
			hql += " and a.status='30' ";
		}
		else{ //回收確認-審核
			hql += " and a.status='31' ";
...
//		}
		...
		System.out.println("[TCBW]-[SDRG0401Q]-[QUERYALL] : " + hql);
		...
		if (getTotalRecord() > 0) 
		{
			...
			{				
				...
				for(Object dtlObj : objList)
				{
					...
					rowArray[5] = Common.get(dtl.getAppName());
					rowArray[6] = Common.get(dtl.getPostDate());
					rowArray[7] = Common.get(dtl.getPostNo());
					rowArray[8] = !"".equals(Common.get(dtl.getMsgsource()))?ecsrMap.get(Common.get(dtl.getMsgsource())):"";
					rowArray[9] = Common.get(dtl.getRecycledeadline());
					rowArray[10] = !"".equals(Common.get(dtl.getHealthbureau1()))?healthMap.get(Common.get(dtl.getHealthbureau1())):"";
					rowArray[11] =  "<div class='discolor'><a href='sdrg0401f.jsp?id="+Common.get(dtl.getId())+"&caseType="+getCaseType()+"'>"+"明細"+"</a></div>";
