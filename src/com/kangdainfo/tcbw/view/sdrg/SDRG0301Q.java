...
public class SDRG0301Q extends SuperBean{
	...
	public Object doQueryAll() throws Exception {		
		...
		if("1".equals(Common.get(getCaseType()))){ //回收中
			hql += " and ( a.status='20' or a.id in (select drg8001Db.id from Drg8004Db where status='40') )";
		}else{ //審核中
			hql += " and a.status='21' ";
		}
		...
		//衛生單位預設查詢條件為自己單位
		if("init".equals(getState()) && "04".equals(c.getCommonDepartment().getDepartmentCode())){
			SDRG0301Q obj = this;
			java.util.Map<String, String> healthMap = TCBWCommon.getMap("select id,unionName from Con1003Db");
			obj.setQ_healthbureau(c.getUserJob());
			obj.setQ_healthbureauName(!"".equals(Common.get(c.getUserJob()))?healthMap.get(Common.get(c.getUserJob())):"");
		}
...
//		}
		...
		System.out.println("[TCBW]-[SDRG0301Q]-[QUERYALL] : " + hql);
		...
		if (getTotalRecord() > 0) 
		{
			...
			{				
				...
				for(Object dtlObj : objList)
				{
					...
					//展延期限
					String extendeadline = View.getLookupField("select extendeadline from Drg8003Db where drg8001Db.id="+Common.getLong(dtl.getId())); 
					//完成回收日期 (若未有完成日期，以系統日代替)
					String appRecDate = !"".equals(Common.get(dtl.getAppRecDate()))?Common.get(dtl.getAppRecDate()):Common.get(Datetime.getYYYMMDD());   				
					
					String light ="";
					String days ="";
					
					if(!"".equals(Common.get(extendeadline))){
						days = Datetime.getDateDiff("d",extendeadline,appRecDate);
					}else{
						days = Datetime.getDateDiff("d",recycledeadline,appRecDate);
					}					
					...
					rowArray[12] =  "<div class='discolor'><a href='sdrg0301f.jsp?id="+Common.get(dtl.getId())+"&caseType="+getCaseType()+"'>"+"明細"+"</a></div>";
