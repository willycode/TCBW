...
public class PROCOND0201F extends SuperBean{
	...
	private String q_docdateS;//文件日期
	private String q_docdateE;//文件日期
	
	@Override
	public Object doQueryOne() throws Exception 
	{
		PROCOND0201F obj = this;
		...
		if(c != null)
		{
			...
			{
				obj.setMedSecCategory(obj.getMedSecCategory());//醫材次類別
	...
	public Object doQueryAll() throws Exception 
	{
		...
		hql += " where maintainman like " + TCBWCommon.likeSqlChar(getUserID()+",");
		
		if(!"".equals(getQ_docdateS()))
		{	
			hql += " and id in (select con3001Db.id from  Con3002Db where docdate >=" + Common.sqlChar( getQ_docdateS())+")";
		}
		
		if(!"".equals(getQ_docdateE()))
		{	
			hql += " and id in (select con3001Db.id from  Con3002Db where docdate <=" + Common.sqlChar( getQ_docdateE())+")";
		}
		
		if(!"".equals(getQ_projType()))
			...
			if (objList != null && objList.size() > 0) 
			{
				...
				{				
					...
					{	
					  ...
					  rowArray[3] = Common.get(MedpKidMap.get(dtl.getPermitKey()))+"字"+Common.get(dtl.getPermitNo())+"號";	
					}
					
					rowArray[4] = Common.get(dtl.getChProduct());
					rowArray[5] = Common.get(dtl.getApplicationName());
					rowArray[6] = Common.get(dtl.getMedSecCategory());
	...
	public String getQ_docdateS() {
		return checkGet(q_docdateS);
	}

	public void setQ_docdateS(String qDocdateS) {
		q_docdateS = checkSet(qDocdateS);
	}

	public String getQ_docdateE() {
		return checkGet(q_docdateE);
	}

	public void setQ_docdateE(String qDocdateE) {
		q_docdateE = checkSet(qDocdateE);
	}
