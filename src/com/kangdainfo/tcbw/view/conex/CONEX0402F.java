...
public class CONEX0402F extends CONEX0401F{
	...
	//判斷是否廠商管理者
	public boolean checkAdmin() throws Exception {
		CommonUser c = ServiceGetter.getInstance().getAuthenticationService().getCurrentUser();
		if(c!=null){
			String isAdmin = View.getLookupField("select isAdmin from Con1006Db where commonUser.id="+Common.getLong(c.getId()));
			if("Y".equals(isAdmin)){
				return true;
			}else{
				return false;
