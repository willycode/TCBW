...
public class CONIN0401F extends CONIN0201F{
	...
	public void updateAuth() {
		try {
			if ("add".equals(getOptype())) {			
				ServiceGetter.getInstance().getCommonService().getCommonAuthDao().updateAuth(getOptype(), getId(), auth, getAuthoritytype(), getEditID());
				setErrorMsg("新增/修改權限成功");
			} else if("remove".equals(getOptype())) {
				ServiceGetter.getInstance().getCommonService().getCommonAuthDao().updateAuth(getOptype(), getId(), authed, getAuthoritytype(), getEditID());
