...
public class TRANSMED03 extends SuperBean
{
...
//			    countyMap.put("臺北市","02");countyMap.put("臺中市","08");countyMap.put("臺南市","14");countyMap.put("臺東縣","17");	
			    ...
			    java.util.List<Med2001Db> updateList = new java.util.ArrayList<Med2001Db>();
			    String wrongString = "";
				int wrong =0;
			    ...
			    {	
                   ...
                   Med2001Db med01 = (Med2001Db)View.getObject("from Med2001Db where applNo="+Common.sqlChar(Common.get(rs[0])));
                   ...
                   if(med01!=null)
                   {
                     ...
                     //notifierType	通報者-屬性
                     String notifierType=Common.get(rs[15]);
                     ...
                     System.out.println("通報者-屬性=="+med01.getNotifierType());//P 通報身分別
...
//                     System.out.println("通報單位所在地區=="+med01.getNotifierCounty());//AW 通報單位所在地區
                     ...
                     med01.setMedTestMedical(Common.get(rs[4]));
                     System.out.println("中文品名=="+med01.getMedTestMedical());////E 醫材品名
...
//                     System.out.println("製造國別=="+med01.getMedCountry());//L 產地
                     ...
                     med01.setPatientId(Common.get(rs[12]));
                     System.out.println("病人識別代號=="+med01.getPatientId());//M 識別代號
...
//					   System.out.println("其他相關資料=="+med01.getOtherRelatedData());//O 問題描述
					   ...
					   if("NI".equals(Common.get(rs[14]))  || Common.get(rs[14]).indexOf("日") == -1) {
                    	   med01.setNotifierRepDate("");
                       } else {
                    	   rs[15] = Common.get(rs[14]).replace("年", "/");
                           rs[15] = Common.get(rs[14]).replace("月", "/");
                           rs[15] = Common.get(rs[14]).replace("日", "");
                           if(checkDate(Common.get(rs[14]))) { // 檢核發生日期為數值或者為作廢字樣	
    						   med01.setNotifierRepDate(Common.get(Datetime.getRocDateTransFromYYYYMMDD(Common.get(rs[14]))));
    					   }
                       }
                       
					   System.out.println("通報日期=="+med01.getNotifierRepDate());//O 通報日期
					   
					   //notifierDept	通報者-服務機構名稱
					   med01.setNotifierDept(Common.get(rs[16]));
					   System.out.println("服務機構名稱=="+med01.getNotifierDept());//Q 服務機構

					   //badReactionSex	不良反應相關資料-性別
					   if("男".equals(Common.get(rs[17]))) {
						   med01.setPatientSex("M");
					   } else if ("女".equals(Common.get(rs[17]))) {
						   med01.setPatientSex("F");
					   }
					   System.out.println("性別=="+med01.getPatientSex());//R 病人性別
					   
					   //badReactionAge	不良反應相關資料-年齡
					   if("#VALUE!".equals(Common.get(rs[18]))) {
						   med01.setPatientAge("");
					   } else {
						   med01.setPatientAge(Common.get(rs[18]));
					   }
					   
					   System.out.println("年齡=="+med01.getPatientAge());//S 年齡
					   
					   //badReactionWeight	不良反應相關資料-體重
					   med01.setPatientWeight(Common.get(rs[19]));
					   System.out.println("體重=="+med01.getPatientWeight());//T	體重
					   
					   //badReactionHeight	不良反應相關資料-身高
					   med01.setPatientHeigth(Common.get(rs[20]));
					   System.out.println("身高=="+med01.getPatientHeigth());//U	身高
					   
					   //badReactionResults	不良反應結果
					   if("先天性畸形".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("04");
					   } else if("危及生命".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("02");
					   } else if("死亡".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("01");
					   } else if("延長病人住院時間".equals(Common.get(rs[21])) || "導致病人住院".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("05");
					   } else if("非嚴重不良反應".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("07");
					   } else if("造成永久性殘廢".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("03");
					   } else if("需作處置以防永久性傷害".equals(Common.get(rs[21]))) {
						   med01.setBadReactionResults("06");
					   }
					   System.out.println("不良反應結果=="+med01.getBadReactionResults());//V	不良反應後果
					   
					   //medModel	懷疑的醫療器材-型號
					   med01.setMedModel(Common.get(rs[22]));
					   System.out.println("型號=="+med01.getMedModel());//W	型號
					   
					   //medNo	懷疑的醫療器材-序號
					   med01.setMedNo(Common.get(rs[23]));
					   System.out.println("序號=="+med01.getMedNo());//X	序號
					   
					   //medLotNum	懷疑的醫療器材-批號
					   med01.setMedLotNum(Common.get(rs[24]));
					   System.out.println("批號=="+med01.getMedLotNum());//Y	批號
					   
					   //medSea	試驗醫師評估醫材與SAE之因果關係	                   
	                   if("確定相關".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("1");
	                   } else if("很可能相關".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("2");
	                   } else if("可能相關".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("3");
	                   } else if("不太可能相關".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("4");
	                   } else if("不相關".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("5");
	                   } else if("無法評估".equals(Common.get(rs[32]))) {
	                	   med01.setMedSea("6");
	                   }
	                   System.out.println("試驗醫師評估醫材與SAE之因果關係=="+med01.getMedSea());//AG 醫材關聯性試驗醫師自評
	                   
					   //procedureSea	試驗醫師評估手續程序與SAE之因果關係	                   
	                   if("確定相關".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("1");
	                   } else if("很可能相關".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("2");
	                   } else if("可能相關".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("3");
	                   } else if("不太可能相關".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("4");
	                   } else if("不相關".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("5");
	                   } else if("無法評估".equals(Common.get(rs[33]))) {
	                	   med01.setProcedureSea("6");
	                   }
	                   System.out.println("試驗醫師評估手續程序與SAE之因果關係=="+med01.getProcedureSea());//AH 試驗醫師評估手續程序與SAE之因果關係
